...
class Gaming:
    ...
    def AlphaBeta(self, board, depth, alpha, beta, currentPlayer):
           ...
           valid_loc=self.get_valid_locations(board)
           check_term=self.is_terminal_node(board)
           if depth==0 or check_term:
               if check_term:
           ...
           if currentPlayer:
               ...
               for col in valid_loc:
                   ...
                   self.drop_piece(b_c,row,col,self.AI_PIECE)
                   new_val=self.AlphaBeta(b_c,depth-1,alpha,beta,False)[1]
                   if new_val>value:
                       value=new_val
                       column=col
               ...
               for col in valid_loc:
                   ...
                   row = self.get_next_open_row(board, col)
                   b_c =board.copy()
                   self.drop_piece(b_c, row, col, self.PLAYER_PIECE)
                   new_val = self.AlphaBeta(b_c, depth - 1, alpha, beta, True)[1]
                   if new_val < value:
                       value = new_val
                       column = col
                   beta = min(beta, value)
